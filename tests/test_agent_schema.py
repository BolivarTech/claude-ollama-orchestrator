...
_CORPUS = {
    "reviewer": {
        ...
        "reject": [
            ...
            {"capability": "reviewer",
             "findings": [{"severity": "nope", "title": "t", "detail": "d"}]},  # bad enum
            {"capability": "reviewer",
             "findings": [{"severity": "info", "title": "t"}]},                 # missing detail
            {"capability": "reviewer", "findings": [], "extra": 1},             # extra top-level key
            {"capability": "reviewer",
             "findings": [{"severity": "info", "title": "t", "detail": "d", "x": 1}]},  # extra item key
            {"capability": "coder", "findings": []},                            # wrong capability const
            {"capability": "reviewer", "findings": {"severity": "info"}},        # findings not a list
            {"capability": "reviewer",
             "findings": [{"severity": "info", "title": 123, "detail": "d"}]},    # title not a string
            {"capability": "reviewer",
             "findings": [{"severity": "info", "title": "t", "detail": ["x"]}]},  # detail not a string
        ],
    },
    "tester": {
        "accept": [
        ...
        "reject": [
            ...
            {"capability": "tester",
             "tests": [{"name": "t", "code": "c", "z": 1}]},                   # extra item key
...
def test_invisibles_stripped_from_all_structured_string_fields():
    ...
    rev = validate_output("reviewer", {"capability": "reviewer", "findings": [
        {"severity": "info", "title": "t" + zwsp + "-1", "detail": "de" + rlo + "tail" + bom}]})
    f = rev["findings"][0]
    assert f["title"] == "t-1"                       # title cleaned, hyphen survives
    assert f["detail"] == "detail"                   # invisibles stripped from detail too
    tst = validate_output("tester", {"capability": "tester", "tests": [
        {"name": "test" + zwsp + "-a", "code": "line1\nli" + zwsp + "ne2"}]})
    t = tst["tests"][0]
...
def test_bidi_isolation_marks_stripped_from_structured_output():
    ...
    tst = validate_output("tester", {"capability": "tester", "tests": [
        {"name": "t", "code": "line1" + lri + "-hidden" + pdi + "\nline2" + rli + fsi + "end"}]})
    ...
    rev = validate_output("reviewer", {"capability": "reviewer", "findings": [
        {"severity": "info", "title": lri + "t-1" + pdi, "detail": "d"}]})
