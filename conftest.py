...
in the format expected by tdd-guard's TestResultSchema:

    {
        "testModules": [
            {
                "moduleId": "tests/test_ollama_backend.py",
                "tests": [
                    {
                        "name": "test_streams_tokens_to_stdout",
                        "fullName": "tests/test_ollama_backend.py::TestStream::test_streams_tokens_to_stdout",
...
# Add the plugin skill's scripts dir to sys.path so tests can import them directly.
_SCRIPTS_DIR = str(Path(__file__).parent / "skills" / "ollama-agents" / "scripts")
